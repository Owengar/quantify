...
class MeasurementControl(Instrument):  # pylint: disable=too-many-instance-attributes
    ...
    def _init(self, name, save_data=True, multi_measurement=False):
        """
        ...
        if not self._dataset or multi_measurement:
    ...
    def run(
        ...
        save_data: bool = True,
        step_function : Optional[None | function] = None,
        empty_run : bool = False,
        multi_measurement: bool = False,
    ) -> xr.Dataset:
        """
        ...
        self._reset(save_data=save_data)
        self._init(name, multi_measurement=multi_measurement)
        ...
        if not empty_run:
            try:
                if self._get_is_batched():
                    ...
                    #CUSTOM: step_function has not been implemented for batched measurements
                    self._run_batched()
                else:
                    if self.verbose():
                        print("Starting iterative measurement...")
                    self._run_iterative(lazy_set, step_function=step_function)
    ...
    def _run_iterative(self, lazy_set: bool = False, step_function : Optional[None | function] = None):
    ...
    def _update(self, print_message: str = None, force_update: bool = False):
        """
