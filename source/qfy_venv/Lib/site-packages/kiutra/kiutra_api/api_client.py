...
import time
from jsonrpclib import ServerProxy, MultiCall
...
class KiutraClient(object):
    ...
    @property
    def server(self):
        return ServerProxy('http://{}:{}'.format(self.host, self.port))
    ...
    def call(self, key, *args, **kwargs):
        ...
        if len(args) >= 0 and len(kwargs) == 0:
            return self.procedure_call(server, server.call, key, *args)
        elif len(kwargs) > 0 and len(args) == 0:
            return self.procedure_call(server, server.call, key=key, **kwargs)
        else:
    ...
    def query(self, key, *args):
        ...
        """
        server = self.server
        return self.procedure_call(server, server.read, key, *args)
    ...
    def set(self, key, value):
        ...
        """
        server = self.server
        return self.procedure_call(server, server.write, key, value)

    def procedure_call(self, server, handle, *args, **kwargs):
        retry = 0
...
if __name__ == '__main__':
    heavy_poll_device = 'T_sample.kelvin'
    kiutra = KiutraClient("192.168.11.20")
    for i in range(1000):
        print(kiutra.query(heavy_poll_device))